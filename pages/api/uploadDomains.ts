    ...
    if (req.method === "POST") {
      ...
      const fileArr = result.files.routes as File[]
       ...
       const array = JSON.parse(reader) as UploadedDomains[];  
        ...
        res.status(200).json({ "message": "Upload successfull", "successCount": response.count });
    }
  } catch (error) {
      console.log(error)
      if (error instanceof APIResponseErr) {
        ...
        const gernicErr = new APIResponseErr(500, ResponseStatus.Failure, new Date().toString(), "Something went wrong")
...
const asyncParse = (req:any): Promise<ParseType> =>
